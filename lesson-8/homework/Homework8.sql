...
select category, count(productid) as num_products from products
...
select*,
case when stockquantity<30 then 'Low_stock'
	 else 'Sufficient' end as Label_stock
from products

--8)Using Customers table, find the total number of customers in each country.
select country, count(customerid) as num_cust from customers
group by country
...
where orderdate>='01/01/2023' and orderdate<'02/01/2023' 
intersect
select customerid from Invoices 
where InvoiceDate not between '01/01/2023' and '02/01/2023'
...
select year(orderdate) as year, avg(TotalAmount) as avg_ord_amt from Orders
...
from Products
group by ProductName,case when price<100 then 'Low'
	 when price between 100 and 500 then 'Mid'
	 else 'High' end 
...
select productid, sum(saleamount) as tot_sale from sales
group by ProductID
...
pivot(
sum(population) for district_name in ([Bektemir],[Chilonzor],[Yakkasaroy])) as pivoted_table
------------------------------------------------------------------------------------------------
--Hard-Level Tasks
...
unpivot(
population for district_name in ([Bektemir],[Chilonzor],[Yakkasaroy])) as unpivoted_table
