...
select count(customerid) as num_rows from customers 
...
select sum(Saleamount) as tot_sales from Sales
where productid=7
...
select departmentname, count(employeeid) as num_employee from employees
...
select customerid, sum(saleamount) as tot_sales from Sales
group by CustomerID
...
select departmentname, count(employeeid) as tot_employee from employees
group by DepartmentName
having count(employeeid)>5
-----------------------------------------------------------------------------------------------
--Medium-Level Tasks (9)
--11)Write a query to calculate the total sales and average sales for each product category from the Sales table.
select ProductID, sum(saleamount) as tot_sales, avg(saleamount) as avg_sales from Sales
group by ProductID

--12)Write a query to count the number of employees from the Department HR.
select count(employeeid) as num_employee from Employees
...
select departmentname, avg(salary) as avg_salary, count(EmployeeID) as tot_employee from Employees
group by DepartmentName
...
select year(SaleDate) as year, sum(saleamount) as tot_sales from sales
group by year(saledate)

--18)Write a query to show the list of customers who placed at least 3 orders.
select customerid, count(orderid) as num_orders from orders
group by CustomerID
...
select departmentname, sum(salary) as tot_salary, avg(salary) as avg_salary from Employees
group by DepartmentName
...
select customerid, sum(totalamount) as tot_sales, min(totalamount) as min_sales from orders
group by CustomerID
...
select  year(orderdate) as year,month(orderdate) as month, sum(totalamount) as tot_sales, count(distinct productid) as unique_products from orders
group by year(OrderDate),month(OrderDate)
having count(distinct productid)>=2

--25)Write a query to find the MIN and MAX order quantity per Year. From orders table.
select year(orderdate) as year, min(quantity) as min_qty, max(quantity) as max_qty from orders
