...
select Courses.CourseName,Students.Name from Courses
cross join Students
...
select Employees.Name from Departments
...
select Orders.OrderID,Payments.PaymentID from Payments
inner join Orders
on Orders.OrderID=Payments.OrderID

--10)Using Orders, Products table Show orders where product price is more than 100.
select Orders.OrderID,Products.ProductName,Products.Price from Orders
inner join Products
...
select Orders.OrderID,Payments.Amount,Orders.TotalAmount from Orders
inner join Payments
on Orders.OrderID=Payments.OrderID
...
select Sales.SaleID,Customers.FirstName,Customers.LastName,Customers.Country from Sales
inner join Customers
on Sales.CustomerID=Customers.CustomerID
where Customers.Country='USA'

--20)Using Orders, Customers table List orders made by customers from 'Germany' and order total > 100.
select Orders.OrderID,Customers.FirstName,Customers.LastName,Customers.Country,Orders.TotalAmount from Orders
inner join Customers
on Orders.CustomerID=Customers.CustomerID
...
select a.Name,a.DepartmentID,b.Name,b.DepartmentID from Employees as a
...
select a.EmployeeID, a.name, a.salary,a.managerid,b.EmployeeID,b.Name,b.ManagerID from Employees as a    --yechilmadi
...
select Customers.FirstName,Customers.LastName from Orders
left join Payments
on Orders.OrderID=Payments.OrderID
left join Customers
on Orders.CustomerID=Customers.CustomerID
